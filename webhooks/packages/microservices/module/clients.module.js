...
let ClientsModule = ClientsModule_1 = class ClientsModule {
    static register(options) {
        ...
        const clients = (clientsOptions || []).map(item => {
            return {
                provide: item.name,
                useValue: this.assignOnAppShutdownHook(client_1.ClientProxyFactory.create(item, 5000)),
            };
        });
        return {
    ...
    static createFactoryWrapper(useFactory) {
        return async (...args) => {
            ...
            const clientProxyRef = client_1.ClientProxyFactory.create(clientOptions, 5000);
